...
- [ ] 20. 实现 CLI（cycms-cli）
  - [ ] 20.1 实现 clap 命令行解析（new / serve / migrate / seed / plugin 子命令）
  - [ ] 20.2 实现 `cycms new <project-name>` 子命令：生成项目骨架（Cargo.toml workspace、目录结构、默认配置文件、示例插件）
  - [ ] 20.3 实现 `cycms serve` 子命令（加载配置 → 初始化 DB → 运行迁移 → 启动 HTTP）
  - [ ] 20.4 实现 `cycms migrate run` / `cycms migrate rollback` 子命令
  - [ ] 20.5 实现 `cycms seed` 子命令（创建初始管理员 + 默认角色 + 默认内容类型）
  - [ ] 20.6 实现 `cycms plugin new <name>` 子命令：生成插件脚手架（Manifest 模板 + 入口代码）
  - [ ] 20.7 实现 `cycms plugin install/enable/disable/remove` 子命令
  - [ ] 20.8 编写 CLI 子命令集成测试
  - _Requirements: 17.1, 17.2, 17.3, 17.4_

- [ ] 21. 实现 AdminWeb（cycms-admin-web）
  - [ ] 21.1 初始化 React 19 + TypeScript + Vite 项目
  - [ ] 21.2 实现 API Client 层（封装 fetch + 认证 token 管理）
  - [ ] 21.3 实现登录页面和认证状态管理
  - [ ] 21.4 实现侧边栏导航布局
  - [ ] 21.5 实现仪表盘页面（内容统计概览 + 快捷操作入口）
  - [ ] 21.6 实现内容类型管理页面（创建/编辑/删除/字段设计器）
  - [ ] 21.7 实现内容管理页面（列表/创建/编辑/发布/版本历史）
  - [ ] 21.8 实现媒体管理页面（上传/浏览/选择器）
  - [ ] 21.9 实现插件管理页面（列表/安装/启停/设置）
  - [ ] 21.10 实现用户/角色与权限矩阵管理页面
  - [ ] 21.11 实现系统设置页面（含插件 settings schema 动态渲染）
  - [ ] 21.12 集成 Vite 构建产物嵌入到 Rust serve 命令（embed 或 SPA fallback）
...
1(骨架) → 2(配置) → 3(数据库) → 4(迁移) → 5(认证) → 6(权限)
                            ...
                            15(插件管理) → 16(Native运行时) → 17(Wasm运行时)
                                            ↘
                            18(API网关) → 19(可观测性) → 20(CLI) → 21(AdminWeb)
```
...
| M6: 接口与交付 | 18-21 | REST API、可观测性、CLI、Admin 前端 |
